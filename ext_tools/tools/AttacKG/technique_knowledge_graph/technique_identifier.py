...
class TechniqueIdentifier:
    ...
    def subgraph_alignment(self, subgraph: set, attack_graph: AttackGraph, max_iter: int = 10000):
        self.node_match_record = {}
        ...
        best_match_score = 0
        best_match_record = {}
        product_iter = itertools.product(*v_list)
        for idx, item in enumerate(product_iter):
            if max_iter is not None and idx >= max_iter:
                break   
        # from tqdm import tqdm
        # for item in tqdm(itertools.product(*v_list)):
...
class AttackMatcher:
    ...
    def attack_matching(self):
        ...
        subgraph_list = nx.weakly_connected_components(self.attack_graph_nx)
